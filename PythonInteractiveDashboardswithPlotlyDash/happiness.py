...
happiness = pd.read_csv('Python-Interactive-Dashboards-with-Plotly-Dash-main/world_happiness.csv')
app = Dash()
...
@app.callback(
    ...
    Output('agerage-div', 'children'),
    Input('submit-button', 'n_clicks'),
    State('country-dropdown', 'value'),
    State('data-radio', 'value')
)
def update_figure(button_click, selected_country, selected_data):

    filtered_happiness = happiness[happiness['country'] == selected_country]

    # Define readable labels
    if selected_data == 'happiness_rank':
        label = 'Happiness Rank'
    else:
        label = 'Happiness Score'

    # Use the label in the title and axis names
    line_fig = px.line(
        filtered_happiness,
        x='year',
        y=selected_data,
        title=f'{label} in {selected_country}',
        labels={selected_data: label, 'year': 'Year'}
    )

    selected_average = filtered_happiness[selected_data].mean()

    return line_fig, f'Average {label} for {selected_country} is {selected_average}'
