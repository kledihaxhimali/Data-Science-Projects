...
app.layout= html.Div([
    ...
    dcc.Input(id='ticker-input', placeholder='Search for symbols from Yahoo Finance', style={'width':'50%'}),
    ...
    dcc.Tabs([
        ...
        dcc.Tab(label='Recent Data', children=[html.Div(id='latest-price-div'),
...
def update_table(button_click, table_interval, ticker):
    if ticker is None:
        ...
        if len(price)>0:
            ...
            return  f"Latest Price for {latest_price} at the time of {latest_time}",price.tail(10).to_dict('records')
        else:
            return f'Nodata for ticker {ticker} on Yahoo Finance', []
